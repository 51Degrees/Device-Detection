...
var patternFileName = '../data/51Degrees-LiteV3.2.dat';
var trieFileName = '../data/51Degrees-LiteV3.2.trie';
...
test('pattern', function (t) {
    var parser = new Parser(patternFileName, properties);
...
test('trie', function (t) {
    var parser = new Parser(trieFileName, properties);
    var ret = parser.parse(userAgent);
    properties.forEach(function (property) {
        t.ok(typeof ret[property] !== undefined, property + '> ok');
    });
...
test('empty userAgent', function (t) {
    var r;
    var psr1 = new Parser(patternFileName, properties);
    r = psr1.parse('');
    t.equal(r.method, 'pattern');

    var psr2 = new Parser(trieFileName, properties);
    r = psr2.parse('');
...
test('memory leak at trie', function (t) {
    var cp1 = process.memoryUsage().rss / 1024 / 1024;
    var psr1 = new Parser(trieFileName, properties);
    ua_array.forEach(function (ua) {
        psr1.parse(ua);
    });
    var cp2 = process.memoryUsage().rss / 1024 / 1024;
