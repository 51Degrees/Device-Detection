...
class Match {
	...
	function overridePropertyValue($propertyName,$valueName) {
		Match_overridePropertyValue($this->_cPtr,$propertyName,$valueName);
	}

	function overrideProfile($profileId) {
		Match_overrideProfile($this->_cPtr,$profileId);
	}
...
class Provider {
	...
	function getAvailablePropertyValueOverrideProperties() {
		$r=Provider_getAvailablePropertyValueOverrideProperties($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new VectorString($r);
	...
	function getDataSetFilePath() {
		return Provider_getDataSetFilePath($this->_cPtr);
	}
