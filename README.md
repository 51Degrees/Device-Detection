...
Trie: A large binary Trie (pronounced Try) populated with device Patterns. Uses a separate data file. Very fast. See [Important note regarding trie](#Important-note-regarding-trie)
...
This is required to build the C/C++ code. For other languages, further details can be found in **README.md** located in each folder.

### Windows
- Visual Studio 2013 or the [C++ Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/) installed.

### Linux
- You will need [CMake 3.4](https://cmake.org/) or greater.
- C++ compiler which supports C++11.
- **Note** : when compiling the examples with GCC or Clang there is a dependency on dmalloc, and dmallocth for the multithreaded reload functions.

## SWIG
All C/C++ based APIs depend on code generated by [SWIG](http://swig.org). The policy is to only support language versions (e.g. Python 2.7 and Python 3.4) which are supported in stable SWIG. Where neccessary development versions will be released in a separate branch using wrappers generated using a development branch of SWIG.

## Included Files

makefile - Builds a command line executable under Linux. 

CMakeLists - Builds the example projects under Windows, Linux and OS X.
...
See [Important note regarding trie](#Important-note-regarding-trie)

&nbsp;&nbsp;&nbsp;&nbsp;51Degrees.c - Placeholder for source code needed to interrogate the data.  
&nbsp;&nbsp;&nbsp;&nbsp;51Degrees.h - Placeholder for the header file for the core detection library. 
